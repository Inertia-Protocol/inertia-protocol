...
- [`packages/keeper/`](packages/keeper/README.md): reference open-source keeper bot. Discovers pending escrows, only acts once the anti-snipe curve makes it genuinely profitable, swap-execution is pluggable -- `mock-dex` (this repo's test program), a real [Orca Whirlpools client](packages/keeper/src/orcaSwap.ts), a real [Raydium CPMM client](packages/keeper/src/raydiumCpmmSwap.ts), and a real [Meteora DLMM client](packages/keeper/src/meteoraDlmmSwap.ts) all exist today
